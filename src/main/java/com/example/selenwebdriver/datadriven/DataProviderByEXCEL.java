...
public class DataProviderByEXCEL<sheetNameInWorkbook> {
    WebDriver driver;
...
//            driver = new ChromeDriver(new ChromeOptions().addArguments("-headless"));
    ...
    // 定义 Excel 文件存放路径。注意：已知的 Excel 文件扩展名 有 2 种，分别是 .xls 和 .xlsx ，需要区分处理
//    public static String excelFilePath = "src/main/java/com/example/selenwebdriver/datadriven/searchWords.xls";
    public static String excelFilePath = "src/main/java/com/example/selenwebdriver/datadriven/searchWords.xlsx";
    // 定义 sheet 工作表名，用于存放测试数据
    public static String sheetNameInWorkbook = "Sheet1";

    @DataProvider(name = "searchWords")
    public static Object[][] Words() throws IOException {
        // 调用 CsvUtll 工具类
//        return CsvUtil.getData(csvFilePath);
        return ExcelUtil.getData(excelFilePath, sheetNameInWorkbook);
    }

    /*
     * 测试逻辑：
     *     （1）打开搜狗首页
     *     （2）从 Excel 文件中读取每行前 2 个逗号分隔的中文词 作为 在搜索输入框中输入的 关键词，两个关键词之间有一个空格
     *     （3）点击 搜索按钮，等待 2 秒，让页面加载完成
     *     （4）验证搜索结果页面是否包含输 Excel 文件中每行第三个中文词，包含则认为测试执行成功，否则失败
     *
     * 注意：
     ...
     *
     * 参考链接：
     *     [YOLO：Java 用 XSSFWorkbook 实现读写 Excel](https://blog.csdn.net/qq_15736701/article/details/116535230)
     *
     *  */
    @Test(dataProvider = "searchWords")
    public void testDataProviderByEXCEL(String searchWord1, String searchWord2, String searchResult) throws InterruptedException {
