...
//        goRobotCtrlV("这是一个 富文本框 ！");
    ...
    *             https://www.nhooo.com/tags/ref-av-dom.html
    *                 https://www.nhooo.com/html5/html5-video.html
    *
    * */
    @Parameters("baseUrl7")
    @Test
    public void playH5Video(String baseUrl7) throws InterruptedException, IOException {
        driver.get(baseUrl7);

        // 定位到 H5 "视频播放器"元素
        WebElement videoPlayer = driver.findElement(By.tagName("video"));
        // 尝试 直接 点击播放，可能会产生报错提示 ...</video> is not clickable at point ...，但即使可以点击，也会仍然播放不取来
//        Assert.assertTrue(videoPlayer.isEnabled());
//        videoPlayer.click();
        ...
        // 获取 网络视频 的 播放时长。注意：不确定返回类型时，或 不想要进行类型转换时，返回类型 可以指定为 Object
        Double videoDuration = (Double) javascriptExecutor.executeScript("return arguments[0].duration;", videoPlayer);
//        Object videoDuration = javascriptExecutor.executeScript("return arguments[0].duration;", videoPlayer);
        System.out.println(videoDuration);

        // 播放 网络视频，调用到 动画播放函数 Animation.play(): void
        //  另外，调用 CSSStyleDeclaration.display: string 也可以用
//        javascriptExecutor.executeScript("arguments[0].display;", videoPlayer);
        javascriptExecutor.executeScript("arguments[0].play()", videoPlayer);
        //  持续播放 5 秒
        Thread.sleep(5000);

        // 暂停播放 网络视频
        javascriptExecutor.executeScript("arguments[0].pause()", videoPlayer);
        //  持续暂停 2 秒
        Thread.sleep(2000);

        // 播放器 切换到 全屏，和 退出全屏
        driver.findElement(By.xpath("//*[@id=\"sl-player-el-video-larkplayer\"]//div[@title=\"全屏\"]")).click();
        Thread.sleep(1000);
        driver.findElement(By.xpath("//*[@id=\"sl-player-el-video-larkplayer\"]//div[@title=\"退出全屏\"]")).click();
        Thread.sleep(1000);

        // 网络视频 截图
        File videoScreenshotFile = ((TakesScreenshot)driver).getScreenshotAs(OutputType.FILE);
        FileUtils.copyFile(videoScreenshotFile, new File("C:\\selenwebdriver_test_screenshot\\videoScreenshot\\videoScreenshotFile.png"));
    }
    ...
    @Parameters("demoUrl3")
    @Test
    public void paintH5Canvas(String demoUrl3) throws InterruptedException, IOException {
        driver.get(demoUrl3);

        /*
        * 调用 JavascriptExecutor 执行 javascript 脚本，绘制一个红色矩形
        *     调用 document.getElementBy... 获取 画布元素
        *     设定 画布为"二维"
        *     设定 画布填充色为"红色"
        *     在画布上 绘制 矩形
        *     截屏保存
        ...
        * */
        JavascriptExecutor javascriptExecutor = (JavascriptExecutor)driver;
        javascriptExecutor.executeScript("var canvas = document.getElementById('canvas');"
                + "var ctx = canvas.getContext('2d');"
                + "ctx.fillStyle='#FF0000';"
                + "ctx.fillRect(0, 0, 150, 150);" );

        File screenshotCanvas = ((TakesScreenshot)driver).getScreenshotAs(OutputType.FILE);
        FileUtils.copyFile(screenshotCanvas, new File("C:\\selenwebdriver_test_screenshot\\canvasScreenshot\\canvasScreenshot.png"));
    }

    /* 15：操作 H5 Storage 存储对象
     *         指 localStorage 和 sessionStorage。
     *         Storage 对象允许在 浏览器中 存取 keyname:keyvalue 键值对的数据。
     *             localStorage 用于长久保存整个网站的数据，保存的数据不会过期，直到手动去删除。 localStorage 属性是只读的。
     *             sessionStorage 只是临时保存同一窗口（或 标签页）的数据 在当前会话 session 中，在关闭窗口（或 标签页）之后，浏览器将会删除这些数据。
     *          详情参阅  https://www.runoob.com/jsref/obj-window.html
     *          另外，H5 的 js 接口文档 网址为 http://html5index.org/
     *
     * */
    @Parameters("demoUrl3")
    @Test
    public void operateH5Storage(String demoUrl3) throws InterruptedException {
        driver.get(demoUrl3);
        JavascriptExecutor javascriptExecutor = (JavascriptExecutor)driver;

        // 使用 Storage 对象 存入 键值对，然后 直接读取
        javascriptExecutor.executeScript("localStorage.setItem('lastname','loo');");
        javascriptExecutor.executeScript("sessionStorage.setItem('familyname','yolo');");
        String lastnameInLocalStorage = (String) javascriptExecutor.executeScript("return localStorage.getItem('lastname')");
        String familynameInSessionStorage = (String) javascriptExecutor.executeScript("return sessionStorage.getItem('familyname')");
        System.out.println("窗口打开时，");
        System.out.println("\tlastnameInLocalStorage: " + lastnameInLocalStorage);
        System.out.println("\tfamilynameInSessionStorage: " + familynameInSessionStorage);
        Assert.assertEquals("loo","loo");
        Assert.assertEquals("yolo","yolo");

        // 使用 Storage 对象 存入 键值对，然后 移除 键值对 再读取
        javascriptExecutor.executeScript("localStorage.setItem('lastname','loo');");
        javascriptExecutor.executeScript("sessionStorage.setItem('familyname','yolo');");
        lastnameInLocalStorage = (String) javascriptExecutor.executeScript("return localStorage.removeItem('lastname');");
        familynameInSessionStorage = (String) javascriptExecutor.executeScript("return sessionStorage.removeItem('familyname');");
        System.out.println("窗口打开时，");
        lastnameInLocalStorage = (String) javascriptExecutor.executeScript("return localStorage.getItem('lastname')");
        familynameInSessionStorage = (String) javascriptExecutor.executeScript("return sessionStorage.getItem('familyname')");
        System.out.println("\tlastnameInLocalStorage: " + lastnameInLocalStorage);
        System.out.println("\tfamilynameInSessionStorage: " + familynameInSessionStorage);
        Assert.assertEquals("","");
        Assert.assertEquals("","");

        // 使用 Storage 对象 存入 键值对，然后 清除 存储对象中 全部数据
        javascriptExecutor.executeScript("localStorage.setItem('lastname','loo');");
        javascriptExecutor.executeScript("sessionStorage.setItem('familyname','yolo');");
        lastnameInLocalStorage = (String) javascriptExecutor.executeScript("return localStorage.clear();");
        familynameInSessionStorage = (String) javascriptExecutor.executeScript("return sessionStorage.clear();");
        System.out.println("清除存储对象中全部数据后，");
        lastnameInLocalStorage = (String) javascriptExecutor.executeScript("return localStorage.getItem('lastname')");
        familynameInSessionStorage = (String) javascriptExecutor.executeScript("return sessionStorage.getItem('familyname')");
        System.out.println("\tlastnameInLocalStorage: " + lastnameInLocalStorage);
        System.out.println("\tfamilynameInSessionStorage: " + familynameInSessionStorage);
        ...
        // 使用 Storage 对象 存入 键值对，然后 关闭窗口 再读取，将会读取不到，报错提示 NoSuchSessionException: invalid session id
        javascriptExecutor.executeScript("localStorage.setItem('lastname','loo');");
        javascriptExecutor.executeScript("sessionStorage.setItem('familyname','yolo');");
        driver.close();
        System.out.println("关闭窗口 再读取，将会读取不到，报错提示 NoSuchSessionException: invalid session id，");
        lastnameInLocalStorage = (String) javascriptExecutor.executeScript("return localStorage.getItem('lastname')");
        familynameInSessionStorage = (String) javascriptExecutor.executeScript("return sessionStorage.getItem('familyname')");
        System.out.println("\tlastnameInLocalStorage: " + lastnameInLocalStorage);
        System.out.println("\tfamilynameInSessionStorage: " + familynameInSessionStorage);
