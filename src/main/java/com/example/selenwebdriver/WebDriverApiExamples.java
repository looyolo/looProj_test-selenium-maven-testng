...
//        driver.get(baseUrl1 + "/");
    ...
    public void getPageInfo(String baseUrl1) {
        ...
        Assert.assertFalse("http://www.sogou.com".equals(currentUrl));
    }
    ...
    public void operateMouse(String demoUrl1) throws InterruptedException {
        ...
        /* 从 iframe 切换回 主界面
         *         这一行代码，不可忽视，一般紧跟着放在 退出 frame 、iframe 之后，作用在于，
         *          切换回主界面后，才可以继续操作主界面的页面元素，其中也包括了其他的 frame 、iframe
         *
         *         存在多个 frame 、 iframe 时，
         *          <1> 习惯定义一个 List 容器来存放，代码示例如下：
         *           List<WebElement> frames = driver.findElement(By.tagName("frame"));
         *          <2> 页面源代码中本身已经添加 frameset 标签 标明 frame 集合，
         *           可以通过 id / name / 索引号 来切换
         */
...
//        WebElement sogouLink = driver.findElement(By.xpath("//a"));
    ...
    /* 13：操作 Javascript 的 alert / prompt / confirm 弹窗
     *         需要使用 Alert 类对象的 getText、accept、dismiss 等方法
     *         需要使用 try/catch 捕捉 NoAlertPresentException 异常，Assert。fail 使得测试程序执行失败
     *
     *         这里不做演示
     *  */

    /* 14：操作 Cookie
     *         主要的：遍历全部 cookie , 全部删除 cookie
     *
     *  */
    @Parameters("baseUrl1")
    @Test
    public void operateCookie(String baseUrl1) {
        driver.get(baseUrl1 + "/");

        // 获取到浏览器中的所有 cookie，不重复地存放到 set 容器中
        Set<Cookie> cookies = driver.manage().getCookies();
        System.out.println("获取到的 cookie 个数：" + cookies.size());

        // 遍历 获取到的所有 cookie
        //  注意：Set 容器 判空 时，不可以是 cookies == null
        if (!(cookies.size() == 0)) {
            System.out.println(String.format("Domain -> Name -> Value -> Value -> Expiry -> Path"));
            for (Cookie cookie : cookies) {
                System.out.println(String.format("%s -> %s -> %s -> %s -> %s",
                        cookie.getDomain(),cookie.getName(),cookie.getValue(),cookie.getExpiry(),cookie.getPath()));
            }
        }
        ...
        // 断言 浏览器 cookie 是否全部被删除
        //  注意：判断条件 不可以是 Assert.assertTrue(driver.manage().getCookies() == null);
